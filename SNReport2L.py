#! python3
#
# SNReport2L.py -- Read the Service Now Assessment Report
# Two Line Report version
#
...
#
messagebox.showinfo("In Excel", 'Remove columns B, D, F, H.\n\nCut Column E and Insert at D.\n\nInsert One column at F.\n\nInsert One column at J.\n\nStarting at column L, Insert 9 Columns (L thru T).\n\nSave as SNReportTemp.xlsx, Overwriting the file!\n\n\n\nExit Excel!!!')
#
...
sheet['e1'] = 'Created RT'
sheet['f1'] = 'Type RT'
sheet['g1'] = 'Description RT'
sheet['h1'] = 'Status RT'
sheet['i1'] = 'PVID RT'
sheet['j1'] = 'Feature RT'
sheet['k1'] = 'Estimate RT'
sheet['l1'] = 'Name FT'
sheet['m1'] = 'Integration FT'
sheet['n1'] = 'Status FT'
sheet['o1'] = 'VP FT'
sheet['p1'] = 'IT Vertical FT'
sheet['q1'] = 'IT Portfolio FT'
sheet['r1'] = 'MD FT'
sheet['s1'] = 'Sponsor Division FT'
sheet['t1'] = 'Sponsor FT'
sheet['u1'] = 'Requested By Name RT'
sheet['v1'] = 'Requested By Email RT'
sheet['w1'] = 'Requested For Name RT'
sheet['x1'] = 'Requested For Email RT'
sheet['y1'] = 'Approve Date FT'
sheet['z1'] = 'Approve Start Date FT'
sheet['aa1'] = 'Approve End Date FT'
sheet['ab1'] = 'Planned Start Date FT'
sheet['ac1'] = 'Planned End Date FT'
sheet['ad1'] = 'Project Manager FT'
sheet['ae1'] = 'Project Analyst FT'
sheet['af1'] = 'Project Category FT'
#
...
for i in range(1,sheet.max_row+1):
    Deliver = sheet.cell(row=i, column=7).value
    if 'Deliver network assessment'in Deliver:
        ...
        sheet.cell(row=i, column=7).value = Deliver4
# Change Estimate
...
root.update()
for i in range(1,sheet.max_row+1):
    Estimate = sheet.cell(row=i, column=7).value
    if 'Build assessment estimate'in Estimate:
        ...
        sheet.cell(row=i, column=6).value = Type
        sheet.cell(row=i, column=7).value = Estimate4
# Change Planning
...
root.update()
for i in range(1,sheet.max_row+1):
    Planning = sheet.cell(row=i, column=7).value
    if 'Resource & Demand planning for'in Planning:
        ...
        sheet.cell(row=i, column=6).value = Type
        sheet.cell(row=i, column=7).value = Planning4
#
...
for i in range(2,sheet.max_row+1):
    Planview = sheet.cell(row=i, column=11).value
    if 'Planview work ID'in Planview:
        Estimate2 = sheet.cell(row=i-1, column=9).value
        if Estimate2 != None:
            sheet.cell(row=i, column=11).value = int(Estimate2)
        else:
...
for i in range(2,sheet.max_row+1):
    ...
    sheet.cell(row=i, column=9).value = int(PVF[0])
    PVF[0] = ""
    PVFTemp = ' '.join(PVF)
    PVFTemp2 = PVFTemp.lstrip()
    if PVFTemp2 != "":
        # Values for the feature cell
        sheet.cell(row=i, column=10).value = PVFTemp2
    else:
        # No values for the feature cell
        sheet.cell(row=i, column=10).value = 'No Feature'
#
...
Output = 'Opening Finacial Info.xlsx'
LABEL = Label(root, text=Output)
LABEL.pack()
root.update()
...
for i in range(2,sheet.max_row+1):
    PVIDFeature = sheet.cell(row=i, column=9).value
    if PVIDFeature == None:
        sheet.cell(row=i, column=9).value = 'No PVID'
        sheet.cell(row=i, column=10).value = 'No Feature'
        continue
    if PVIDFeature == "":
        sheet.cell(row=i, column=9).value = 'No PVID'
...
for i in range(1,sheet.max_row):
    PVIDRT = str(sheet.cell(row=i, column=9).value)
    if  PVIDRT == 'PVID RT':        # Skip if column header
        ...
        if PVIDFeature != 'None':
            if PVIDFeature == 'F3607':
                sheet.cell(row=i, column=12).value = 'NAIA'
                continue
            if PVIDFeature == 'F9831':
                sheet.cell(row=i, column=12).value = 'WAN-LAN'
                continue
            if PVIDFeature == 'F20206':
                sheet.cell(row=i, column=12).value = 'NAIA - CRE'
                continue
            if PVIDFeature == 'F20207':
                sheet.cell(row=i, column=12).value = 'NAIA - Network Engineering'
                continue
            if PVIDFeature == 'F20261':
                sheet.cell(row=i, column=12).value = 'NAIA - Internal'
                continue
            if PVIDFeature == 'F20268':
                sheet.cell(row=i, column=12).value = 'No Description'
                continue
            else:
                sheet.cell(row=i, column=12).value = 'No 8601 Feature'
                continue
    elif str(PVIDRT) == '8576':     # PVID is 8576 compare Feature and set the Name
        PVIDFeature = sheet.cell(row=i, column=10).value
        if PVIDFeature != 'None':
            if PVIDFeature == 'F20205':
                sheet.cell(row=i, column=12).value = 'NAIA - Operational Necessity'
                continue
            if PVIDFeature == 'F20259':
                sheet.cell(row=i, column=12).value = 'Application Definition'
                continue
            if PVIDFeature == 'F20260':
                sheet.cell(row=i, column=12).value = 'SOTA'
                continue
            if PVIDFeature == 'F':
                sheet.cell(row=i, column=12).value = 'Feature Not Defined'
                continue
            else:
                sheet.cell(row=i, column=12).value = 'No 8756 Feature'
                continue
    if PVIDRT !="PVID RT":        # Scan the Finance Info file
        for j in range(1,fsheet.max_row+1):
            ...
            if PVIDRT in PVIDFT:    #Look to see if the PVIDRT in in the PVIDFT
                sheet.cell(row=i, column=12).value = fsheet.cell(row=j, column=4).value     #Name
                sheet.cell(row=i, column=13).value = fsheet.cell(row=j, column=7).value     #Integration
                sheet.cell(row=i, column=14).value = fsheet.cell(row=j, column=9).value     #Status
                sheet.cell(row=i, column=15).value = fsheet.cell(row=j, column=11).value    #VP
                sheet.cell(row=i, column=16).value = fsheet.cell(row=j, column=12).value    #IT Vertical
                sheet.cell(row=i, column=17).value = fsheet.cell(row=j, column=13).value    #IT Portfolio
                sheet.cell(row=i, column=18).value = fsheet.cell(row=j, column=14).value    #MD
                sheet.cell(row=i, column=19).value = fsheet.cell(row=j, column=15).value    #Sponsor Division
                sheet.cell(row=i, column=20).value = fsheet.cell(row=j, column=16).value    #Sponsor
                sheet.cell(row=i, column=25).value = fsheet.cell(row=j, column=17).value    #Approve Date
                sheet.cell(row=i, column=26).value = fsheet.cell(row=j, column=18).value    #Approve Start
                sheet.cell(row=i, column=27).value = fsheet.cell(row=j, column=19).value    #Approve End
                sheet.cell(row=i, column=28).value = fsheet.cell(row=j, column=20).value    #Planned Start
                sheet.cell(row=i, column=29).value = fsheet.cell(row=j, column=21).value    #Planned End
                sheet.cell(row=i, column=30).value = fsheet.cell(row=j, column=28).value    #Project Manager
                sheet.cell(row=i, column=31).value = fsheet.cell(row=j, column=36).value    #Project Analyst
                sheet.cell(row=i, column=32).value = fsheet.cell(row=j, column=39).value    #Project Category
for i in range(2,sheet.max_row+1):
    FinanceName = sheet.cell(row=i, column=12).value
    if FinanceName == None:
        sheet.cell(row=i, column=12).value = 'xxxxxxxxxx'
        continue
    if FinanceName == "":
        sheet.cell(row=i, column=12).value = 'xxxxxxxxxxz'
#
...
for col in sheet.columns:
     ...
     for cell in col:
         ...
         if column == 'F':                                      #Format Type
             cell.alignment = Alignment(horizontal='center')
         if column == 'G':                                      # Format Description RT
             cell.alignment = Alignment(wrap_text='true')
         if column == 'I':                                      # Format PVID RT
             cell.alignment = Alignment(horizontal='center', wrap_text='true')
         if column == 'J':                                      # Format Feature RT
             cell.alignment = Alignment(horizontal='center', wrap_text='true')
         if column == 'K':                                      # Format Estimate RT
             cell.alignment = Alignment(horizontal='center')
         if column == 'Y':                                      # Format Approve Date FT
             cell.alignment = Alignment(horizontal='center')
             cell.number_format = 'mm/dd/yy'
         if column == 'Z':                                      # Format Approve Start Date FT
             cell.alignment = Alignment(horizontal='center')
             cell.number_format = 'mm/dd/yy'
         if column == 'AA':                                      # Format Approve End Date FT
             cell.alignment = Alignment(horizontal='center')
             cell.number_format = 'mm/dd/yy'
         if column == 'AB':                                     # Format Planned Start Date FT
             cell.alignment = Alignment(horizontal='center')
             cell.number_format = 'mm/dd/yy'
         if column == 'AC':                                     # Format Planned End Date FT
             cell.alignment = Alignment(horizontal='center')
             cell.number_format = 'mm/dd/yy'
...
sheet.column_dimensions['F'].width = 12                 # Type RT
sheet.column_dimensions['G'].width = 76                 # Description RT
sheet.column_dimensions['H'].width = 18                 # Status RT
sheet.column_dimensions['I'].width = 24                 # PVID RT
sheet.column_dimensions['J'].width = 20                 # Feature RT
sheet.column_dimensions['K'].width = 16                 # Estimate RT
#
...
#
Output = 'Removing temporary filesa'
LABEL = Label(root, text=Output)
LABEL.pack()
root.update()
